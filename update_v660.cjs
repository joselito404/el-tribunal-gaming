...
c = c.replace(/const SINGLE_PLAYER_GAMES = \[([\s\S]*?)\];/, (match, gamesStr) => {
  let updatedGames = gamesStr;
  
  // Regex to match each game block independently
  let blocks = updatedGames.split(/(\{\s*id:\s*['"][^'"]+['"][\s\S]*?\})/g);
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    if (block.trim().startsWith('{')) {
      ...
      if (idMatch) {
         ...
         // First, extract the app ID from headerImg or any steam URL to construct the perfect image
         let appIdMatch = block.match(/store_item_assets\/steam\/apps\/(\d+)\//) || block.match(/steam\/apps\/(\d+)\//);
         if (appIdMatch) {
             let appId = appIdMatch[1];
             let newImgUrl = `https://shared.akamai.steamstatic.com/store_item_assets/steam/apps/${appId}/library_600x900_2x.jpg`;
             block = block.replace(/img:\s*['"].*?['"]/, `img: "${newImgUrl}"`);
         }

         // 2. UPDATE DESCRIPTION to Premium
         if (descriptions[id]) {
             let descText = descriptions[id];
             // Escape quotes safely
             descText = descText.replace(/"/g, '\\"');
             
             // Replace description
             if (block.includes('description:')) {
                 block = block.replace(/description:\s*['"]((?:\\.|[^'"])*?)['"]/, `description: "${descText}"`);
             } else if (block.includes('summary:')) {
                 block = block.replace(/summary:\s*['"]((?:\\.|[^'"])*?)['"]/, `description: "${descText}"`);
             } else {
                 block = block.replace(/genres:/, `description: "${descText}",\n    genres:`);
             }
         }
      }
      blocks[i] = block;
  ...
  return "const SINGLE_PLAYER_GAMES = [" + blocks.join('') + "];";
});
              ...
              <div className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                ...
                <div className="w-[calc(100%-4rem)] md:w-[calc(50%-3rem)] bg-gray-800/60 p-5 md:p-8 rounded-3xl border border-amber-600/30 shadow-[0_0_20px_rgba(217,119,6,0.1)] transition-all">
                  ...
                  <p className="text-sm text-gray-400 mt-2">
                    Reemplazo integral de la base de datos de juegos. Limpieza masiva de assets asegurando portadas verticales (v2x) para toda la galería para eliminar distorsiones. Expansión de 22 descripciones adicionales a calidad narrativa editorial premium.
                  </p>
                </div>
              </div>
...
c = c.replace(/(\{\/\* v6\.6\.0 \*\/\})/, '{/* temp block */}'); // In case it replaced the first one by accident
c = c.replace(/(\{\/\* v6\.5\.0 \*\/\})/, newChangelog + '$1');

fs.writeFileSync('src/App.jsx', c, 'utf8');
console.log('v6.6.0 update applied to all 32 games.');
